...
import { tomorrowDate } from "../helpers/countTomorrowDate";
  ...
  useEffect(() => {
    const fetchFoods = async () => {
      ...
      if (isExpires) {
        ...
        setFoods(data.food);
        tomorrow.current = tomorrowDate(now);
      }
    };
    fetchFoods();
  }, [currentShift]);
  return (
    <div className="max-w-6xl mx-auto px-3 flex flex-col">
      ...
      <form className="flex items-center text-xl justify-evenly my-6 sm:text-xl sm:justify-center sm:gap-20 select-none">
        <label
          className={`${
            currentShift === "first" && "active rounded-lg font-bold"
          } p-2`}
          htmlFor="firstShift"
        >
          <input
        ...
        <label
          className={`${
            currentShift === "second" && "active rounded-lg font-bold"
          } p-2`}
          htmlFor="secondShift"
        >
          <input
      ...
      {foods && foods.length > 0 ? (
        <div className="flex flex-wrap gap-4 mb-3 min-w-[70%] sm:min-w-[60%] mx-auto select-none">
          {foods.map((food) => (
            <FoodCard
              key={food._id}
              food={food}
